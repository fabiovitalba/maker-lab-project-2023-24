...
def item_list_window(win_height, win_width, button_font, items_df):
    ...
    back_button = ctk.CTkButton(il_window, text="Back", command=il_window.destroy,
    ...
    for index, row in items_df_reduced.iterrows():
        days_to_exp = (pd.to_datetime(
            row[EXP_LBL], unit="s") - datetime.now()).days
        if days_to_exp < 0:
    ...
    items_df_reduced[EXP_LBL] = pd.to_datetime(
        items_df_reduced[EXP_LBL], unit="s").dt.strftime("%B %d, %Y")
    items_df_reduced[QTY_LBL] = items_df_reduced[QTY_LBL].astype(int)

    table_values = [items_df_reduced.columns.tolist()] + \
        items_df_reduced.values.tolist()
    table = CustomCTkTable(master=il_window, row=len(
        table_values), column=3, values=table_values, font=TABLE_ITEM_FONT, row_colors=row_colors)
