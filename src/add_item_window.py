...
def add_item_window(win_height, win_width, button_font, items_df):
    ...
    def on_barcode_change(event):
        ...
        if (event.keysym == "Return") and (barcode != ""):
            ...
            if description == "":
                ...
                expiration_date_entry.insert(
                    0, str(curr_exp_date_list[0].strftime("%d/%m/%Y")))
    ...
    def on_exp_date_change(event, exp_date_list):
        ...
        next_input = False
        if event.keysym == "Return":
            [exp_date_list[0], next_input] = handle_exp_date_input(
                exp_date_text, exp_date_list[0])
            if next_input:
                confirm_input()
            else:
                expiration_date_entry.delete(0, ctk.END)
                expiration_date_entry.insert(
                    0, str(exp_date_list[0].strftime("%d/%m/%Y")))
                expiration_date_entry.focus()
    ...
    def confirm_input():
        ...
        new_item = [barcode_value, description_value, pd.to_datetime(
            curr_exp_date_list[0]).timestamp(), quantity_value, pd.to_datetime("now").timestamp()]
    ...
    description_label = ctk.CTkLabel(
        description_frame, text="Description:", width=label_width, height=label_height, font=label_font)
    description_label.pack(side=ctk.LEFT, padx=10)
    description_entry = ctk.CTkEntry(
        description_frame, width=input_width, height=input_height, font=input_font)
    ...
    plus_button = ctk.CTkButton(
        quantity_frame, text="+", command=lambda: update_quantity(1.0), font=input_font, height=button_height)
    ...
    expiration_date_label = ctk.CTkLabel(
        expiration_date_frame, text="Expiration Date (dd/mm/yyyy):", width=label_width, height=label_height, font=label_font)
    expiration_date_label.pack(side=ctk.LEFT, padx=10)
    expiration_date_entry = ctk.CTkEntry(
        expiration_date_frame, width=input_width, height=input_height, font=input_font)
    expiration_date_entry.bind("<Key>", lambda event: on_exp_date_change(
        event=event, exp_date_list=curr_exp_date_list))
