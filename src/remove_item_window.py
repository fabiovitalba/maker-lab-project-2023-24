...
# Creates a "Remove Item"-view on top of the current View that allows the user to remove Items partially or completely from their Inventory.
# The user may decide to use the "Barcode"-input or not. When first using the barcode, the user will only be able to select items with the
# provided barcode.
# When not using the barcode, the User may select any item to be removed. Using the left and right arrow-key-buttons the user can iterate
# over the Item Inventory. 
# Once an Item is selected, the user can use the + and - buttons to alter the quantity TO BE REMOVED from the selected item. If the full
# quantity (or more) is selected, the whole item is removed.
# Using the "CONFIRM"-button, the action will be applied to the local and cloud Item Inventory.
def remove_item_window(win_height, win_width, button_font, items_df):
    ...
    # Used from the "Barcode"-input, filters the available item list if the Barcode is selected.
    def on_barcode_change(event, item_row_index_list):
        barcode = barcode_entry.get()
        if event.keysym == "Return":
            confirm_barcode_input(item_row_index_list)

    # Returns the list of available items for selection. If a Barcode was provided, the list is filtered by barcode.
    # If none was provided, the whole list is returned.     
    ...
    # Updates the item label to show information about the currently selected item.
    def update_item_label(items_df, item_row_index):
        ...
        item_exp = pd.to_datetime(selected_items.iloc[item_row_index][EXP_LBL], unit="s").strftime("%B %d, %Y")
        quantity_entry.set(1)
        label.configure(text=f"{item_desc}\n{item_qty:.0f} units\nexpires on: {item_exp}")
        confirm_button.configure(state="normal")

    # Updates the currently selected Item's index, used when pressing the arrow-key-buttons
    ...
    # Used when providing a barcode. Checks the list for how many items exist with the given barcode.
    # Only if more than 1 exists, the user has to select the correct item, otherwise no selection is
    # required.
    ...
    # Confirms the form's data by removing the provided quantity (or whole item) from the user's Item Inventory.
    def confirm_input():
        selected_items = get_selected_items(items_df)
        remove_item(items_df, selected_items.iloc[selected_item_index].index, float(quantity_entry.get()))
    ...
    right_arrow_button = ctk.CTkButton(barcode_frame, text="▶", command=lambda: move_right(item_row_index_list=selected_item_index), font=button_font, height=button_height, width=button_height)
    left_arrow_button = ctk.CTkButton(barcode_frame, text="◀", command=lambda: move_left(item_row_index_list=selected_item_index), font=button_font, height=button_height, width=button_height)
    ...
    label = ctk.CTkLabel(label_frame, text="", font=label_font, width=890, height=200)
    ...
    plus_button = ctk.CTkButton(quantity_frame, text="+", command=lambda: update_quantity(1.0), font=input_font, height=button_height, width=button_height)
    plus_button.pack(side=ctk.LEFT)

    minus_button = ctk.CTkButton(quantity_frame, text="-", command=lambda: update_quantity(-1.0), font=input_font, height=button_height, width=button_height)
